...
public class IsolationLevelExercise {
    ...
    private void createTables(Connection conn) {
        try {
            conn.createStatement().execute("create table bids (id BIGINT NOT NULL auto_increment, user VARCHAR(50), time TIMESTAMP , amount BIGINT, currency VARCHAR(20), PRIMARY KEY (id))");
            conn.createStatement().execute("create table items (id BIGINT NOT NULL auto_increment, name VARCHAR(50), PRIMARY KEY (id))");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
